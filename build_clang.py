...
compile_flags = "-std=c++20 -dynamic -Wall -Wextra "
optim_flags = " -O3 -march=avx2" # trouver les veclib
debugging_flags = " -gcodeview"

link_flags = "-fuse-ld=lld"
if release: link_flags += " "
if debug: link_flags += " "
...
def build_juce(sources: str, out_path: str, flags: str) -> None:
    ...
    command = f"clang++ {flags} -c {defines} {includes} {juce_sources}"
    Print(command)
...
command = f"cl {command_flags} -o {plugin_name}.vst3 {defines} {includes} {plugin_sources} {link_flags} {juce_objects} {libs}"
