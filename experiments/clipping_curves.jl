...
cubic_clip(x) =  x < -1.0 ? -2.0/3.0 : x > 1.0 ? 2.0/3.0 : x - 1.0/3.0 * x^3

cubic_clip_approx(x) = x < -1.0 ? -0.6666 : x - 0.3333 * x^3

cubic_clip_norm(x, scale= 1.0) = scale * 1.5 * cubic_clip(x * 2.0/(3.0 * scale))

atan_hard_clip(x) = x > 0 ? 2/π * atan(x * π/2) : x < 1.3 ? 1.3 : x

tanh_cubic(x) = x > 0 ? tanh(x) : cubic_clip_norm(x)
